...
export default function RaceSimulation() {
  ...
  // ─── Animation loop ───────────────────────────────────
  const getDriverStatesAt = useCallback((ms) => {
  ...
  const drawCanvas = (states) => {
    ...
    const minY = Math.min(...allY),
      maxY = Math.max(...allY);
    const PAD = 40;
    const scaleX = (v) => PAD + ((v - minX) / (maxX - minX)) * (W - PAD * 2);
    const scaleY = (v) => PAD + ((v - minY) / (maxY - minY)) * (H - PAD * 2);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isLoaded && (
          <>
            <div style={{ marginTop: 24, fontSize: 11, color: "#888", marginBottom: 8 }}>
              DRIVERS
            </div>
            {driverStates
        ...
        {isLoaded && (
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <button style={ctrlBtn} onClick={() => setCurrentMs(minMs)}>
                ⏮
              </button>
              <button
                style={{ ...ctrlBtn, background: "#e10600" }}
              ...
              <button
                style={ctrlBtn}
                onClick={() => setCurrentMs((prev) => Math.min(prev + 30000, maxMs))}
              >
                ⏭
              </button>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                {[0.5, 1, 2, 4].map((s) => (
                  <button
                    ...
                    style={{
                      ...ctrlBtn,
                      background: speed === s ? "#333" : "transparent",
                      fontSize: 11,
                    }}
                    onClick={() => setSpeed(s)}
                  >
          ...
          >
            <div
              style={{
                ...
                height: 12,
                borderRadius: "50%",
                background: selectedDriverState.color,
              }}
            />
            <span style={{ fontWeight: "bold", fontSize: 14 }}>
              {selectedDriverState.code}
            </span>
          </div>

          <TelemetryRow
            label="SPEED"
            value={`${Math.round(selectedDriverState.speed)} km/h`}
          />
          <TelemetryRow label="GEAR" value={String(selectedDriverState.gear)} />
          <TelemetryRow
            label="THROTTLE"
            value={`${Math.round(selectedDriverState.throttle)}%`}
          />
          <TelemetryRow
            label="BRAKE"
            value={selectedDriverState.brake ? "ON" : "OFF"}
            color={selectedDriverState.brake ? "#e10600" : "#0f0"}
          />
          <TelemetryRow
            label="DRS"
            value={selectedDriverState.drs >= 10 ? "OPEN" : "CLOSED"}
            color={selectedDriverState.drs >= 10 ? "#0f0" : "#888"}
          />
          <TelemetryRow label="LAP" value={String(selectedDriverState.lap_number)} />
          <TelemetryRow
            label="TYRE"
            value={selectedDriverState.compound}
            color={
              {
                SOFT: "#FF3333",
                MEDIUM: "#FFD700",
                ...
                INTERMEDIATE: "#39B54A",
                WET: "#0067FF",
              }[selectedDriverState.compound] || "#FFF"
            }
          />
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ height: 6, background: "#222", borderRadius: 3 }}>
              <div
                style={{
                  height: "100%",
                  width: `${selectedDriverState.throttle}%`,
                  background: "#0f0",
                  borderRadius: 3,
                }}
              />
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ height: 6, background: "#222", borderRadius: 3 }}>
              <div
                style={{
                  height: "100%",
                  width: `${Math.min((selectedDriverState.speed / 350) * 100, 100)}%`,
                  background: "#e10600",
                  borderRadius: 3,
                }}
              />
...
// ─── Small helpers ─────────────────────────────────────
function TelemetryRow({ label, value, color = "#FFF" }) {
  return (
